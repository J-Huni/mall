...
<body style="font-family: fantasy;">
	<div>
		...
		<c:choose>
			<c:when test='${sessionScope.id == "admin" }'>
				<a href="/mall/adPage/admin" style="float: right;margin-top: 3%;margin-right: 10%;">관리자 페이지</a>
			</c:when>
		</c:choose>
	</div>
	<div style="margin-left: 81%;">
		<form action="/mall/modifyID" method="get">
			<input name="id" value="${sessionScope.id}" type="hidden">
			<button type="submit" id="modify" style="display:none">개인정보 수정</button>
		</form>
		<form action="/mall/buylist" method="get">
			<input value="${sessionScope.id}" type="hidden">
			<button id="buylist" style="display:none;width: 100px;">구매 내역</button>
		</form>
	</div>
	<div id="top">
		<ul
			...
			<c:choose>
				<c:when test='${session == "yes" }'>
					<li>
						<a href="#" id="myacount" style="padding-bottom: 0px; color: #A84E19;">
						${sessionScope.id}님</a>
					</li>
					<li><a href="/logout"
	...
	<div>
		<input type="hidden" id="modifyID" name="id" value="${sessionScope.id}">
	</div>
	<div>
